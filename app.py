...
PERSONAS = {
    "claude":     {"initial":"You are Claude by Anthropic. Careful reasoning, nuance, intellectual honesty. Be thorough yet clear. 3-5 paragraphs.","debate":"You are Claude. Engage in genuine discussion. Agree where right, challenge where wrong, add missing angles. 2-4 paragraphs.","followup":"You are Claude in an ongoing discussion. Respond thoughtfully, referencing prior discussion. 2-3 paragraphs."},
...
def call_openai_compat(api_key, base_url, model, system, message, image_data=None):
    ...
    r = requests.post(f"{base_url}/chat/completions",json={"model":model,"messages":[{"role":"system","content":system},{"role":"user","content":content}],"max_tokens":1000},headers={"Authorization":f"Bearer {api_key}","Content-Type":"application/json"},timeout=60)
    r.raise_for_status(); return r.json()["choices"][0]["message"]["content"]

def call_perplexity(system, message, image_data=None):
    if not PERPLEXITY_KEY: raise ValueError("No Perplexity key")
    r = requests.post("https://api.perplexity.ai/chat/completions",json={"model":"llama-3.1-sonar-small-128k-online","messages":[{"role":"system","content":system},{"role":"user","content":message}]},headers={"Authorization":f"Bearer {PERPLEXITY_KEY}","Content-Type":"application/json"},timeout=60)
    r.raise_for_status(); return r.json()["choices"][0]["message"]["content"]
...
def ai_card_html(persona,text,label=""):
    ...
    lbl=f'<span style="margin-left:auto;font-size:9px;color:#3A4A5A;background:#111820;padding:2px 6px;border-radius:3px">{label}</span>' if label else ""
...
def phase_header(num,title,state):
    ...
    st.markdown(f'<div style="display:flex;align-items:center;gap:12px;margin:20px 0 14px 0"><span style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:50%;background:{bg};border:1.5px solid {border};font-size:12px;font-weight:bold;color:{tc}">{icon}</span><span style="font-size:11px;font-weight:bold;letter-spacing:0.1em;color:{tc};font-family:monospace">{title}</span></div>',unsafe_allow_html=True)

def divider(): st.markdown('<hr style="border:none;border-top:1px solid #111E2E;margin:24px 0">',unsafe_allow_html=True)
...
def key_status_banner():
    ...
    parts=[f'<span style="color:#3DFFC0">✓ {n}</span>' if k else f'<span style="color:{"#FF6B6B" if r else "#3A5060"}">✗ {n} ({"REQUIRED" if r else "simulated"})</span>' for n,k,r in checks]
    st.markdown(f'<div style="font-size:11px;padding:10px 14px;background:#090E18;border-radius:6px;border-left:3px solid #1A3A5A;margin-bottom:18px;font-family:monospace">API KEYS &nbsp;·&nbsp; {" &nbsp;·&nbsp; ".join(parts)}</div>',unsafe_allow_html=True)
...
def render_context_panel():
    with st.expander("📎  ADD CONTEXT — documents, images, links  (optional)",expanded=False):
        ...
        with c1:
            ...
            uploaded=st.file_uploader("up",type=["pdf","png","jpg","jpeg","webp","gif"],accept_multiple_files=True,label_visibility="collapsed")
        with c2:
    ...
    if uploaded:
        for f in uploaded:
            ...
            if f.type=="application/pdf": text_items.append({"label":f"PDF:{f.name}","content":extract_pdf_text(fb)}); summary.append(f"📄 {f.name}")
                ...
                b64=base64.b64encode(fb).decode(); image_data.append({"media_type":f.type,"data":b64,"name":f.name})
                text_items.append({"label":f"Image:{f.name}","content":f"[Image '{f.name}' provided]"}); summary.append(f"🖼 {f.name}")
    if url_input and url_input.strip().startswith("http"):
        with st.spinner("Fetching URL..."): text_items.append({"label":f"URL:{url_input}","content":fetch_url_text(url_input.strip())})
        summary.append(f"🔗 {url_input[:35]}...")
    text_ctx=build_text_context(text_items); ctx_summary=", ".join(summary)
    if ctx_summary: st.markdown(f'<div style="font-size:10px;color:#3DFFC0;padding:6px 12px;background:rgba(61,255,192,0.05);border:1px solid rgba(61,255,192,0.2);border-radius:6px;margin-top:6px;font-family:monospace">✓ Context: {ctx_summary}</div>',unsafe_allow_html=True)
...
def render_share_panel(question,synthesis,followups):
    ...
    with c1:
        ...
        st.components.v1.html(f'<button onclick="navigator.clipboard.writeText(`{esc}`).then(()=>{{this.innerText=\'✓ COPIED!\';this.style.color=\'#3DFFC0\';setTimeout(()=>{{this.innerText=\'📋 COPY\';this.style.color=\'#7BA7FF\';}},2000)}})" style="width:100%;background:rgba(123,167,255,0.15);color:#7BA7FF;border:1px solid rgba(123,167,255,0.4);border-radius:6px;padding:8px 4px;font-family:monospace;font-size:11px;font-weight:bold;cursor:pointer">📋 COPY</button>',height=42)
    with c2: st.download_button("⬇ DOWNLOAD",data=full,file_name="nexus-discussion.md",mime="text/markdown",use_container_width=True)
    with c3: st.markdown(f'<a href="mailto:?subject={enc_s}&body={enc_b}" target="_blank"><button style="width:100%;background:rgba(123,167,255,0.15);color:#7BA7FF;border:1px solid rgba(123,167,255,0.4);border-radius:6px;padding:8px 4px;font-family:monospace;font-size:11px;font-weight:bold;cursor:pointer">📧 EMAIL</button></a>',unsafe_allow_html=True)
    with c4: st.markdown(f'<a href="https://wa.me/?text={enc_wa}" target="_blank"><button style="width:100%;background:rgba(123,167,255,0.15);color:#7BA7FF;border:1px solid rgba(123,167,255,0.4);border-radius:6px;padding:8px 4px;font-family:monospace;font-size:11px;font-weight:bold;cursor:pointer">💬 WHATSAPP</button></a>',unsafe_allow_html=True)
    with c5: st.markdown(f'<a href="https://twitter.com/intent/tweet?text={enc_tw}" target="_blank"><button style="width:100%;background:rgba(123,167,255,0.15);color:#7BA7FF;border:1px solid rgba(123,167,255,0.4);border-radius:6px;padding:8px 4px;font-family:monospace;font-size:11px;font-weight:bold;cursor:pointer">𝕏 TWITTER</button></a>',unsafe_allow_html=True)

def render_history_sidebar():
    with st.sidebar:
        st.markdown('<div style="font-size:14px;font-weight:bold;letter-spacing:0.12em;color:#E0E8F5;margin-bottom:4px;font-family:monospace">⬡ NEXUS AI</div><div style="font-size:9px;color:#3A5A7A;letter-spacing:0.1em;margin-bottom:20px;font-family:monospace">DISCUSSION HISTORY</div>',unsafe_allow_html=True)
        ...
        if not history:
            st.markdown('<div style="font-size:11px;color:#2A3A4A;padding:12px 0;font-family:monospace">No discussions yet.<br>Start one above!</div>',unsafe_allow_html=True); return
        ...
        for gname,items in groups.items():
            ...
            for disc in items:
                ...
                with ci:
                    st.markdown(f'<div style="background:{bg};border-left:{bl};padding:8px 10px;border-radius:6px;margin-bottom:4px;font-family:monospace"><div style="font-size:11px;color:{"#E0E8F5" if is_active else "#8090A8"};line-height:1.4">{q_short}</div><div style="font-size:9px;color:#2A3A4A;margin-top:3px">{tstr}{fu_badge}</div></div>',unsafe_allow_html=True)
                ...
                with cd:
                    if st.button("✕",key=f"del_{disc_id}",help="Delete"):
                        ...
                        st.rerun()
        st.markdown('<hr style="border:none;border-top:1px solid #0D1820;margin:20px 0 10px 0"><div style="font-size:9px;color:#1E2E3E;line-height:1.6;font-family:monospace">History stored in nexus_history.json.<br>On Streamlit Cloud, use Supabase for cross-deploy persistence.</div>',unsafe_allow_html=True)

def _load_discussion(disc):
    for k,v in [("active_id",disc.get("id","")),("phase",disc.get("phase",4)),("question",disc.get("question","")),("r1",disc.get("r1",{})),("r2",disc.get("r2",{})),("synthesis",disc.get("synthesis","")),("followups",disc.get("followups",[])),("context_summary",disc.get("context_summary",""))]:
        st.session_state[k]=v

def _current_discussion():
    return {"id":st.session_state.get("active_id",str(uuid.uuid4())),"created_at":st.session_state.get("created_at",datetime.utcnow().isoformat()),"question":st.session_state.get("question",""),"phase":st.session_state.get("phase",0),"r1":st.session_state.get("r1",{}),"r2":st.session_state.get("r2",{}),"synthesis":st.session_state.get("synthesis",""),"followups":st.session_state.get("followups",[]),"context_summary":st.session_state.get("context_summary","")}
...
def main():
    ...
    if st.session_state.phase==0 and not start:
        st.markdown('<div style="padding:20px;background:#090E18;border-radius:10px;border:1px solid #111E2E;font-family:monospace"><div style="font-size:9px;letter-spacing:0.12em;color:#3A5060;margin-bottom:14px">PARTICIPATING MODELS</div><div style="display:flex;flex-wrap:wrap;gap:20px;margin-bottom:14px"><span><b style="color:#FF8B4E">● Claude</b> <span style="color:#2A3A4A;font-size:10px">— nuance & honesty</span></span><span><b style="color:#7BA7FF">● Gemini</b> <span style="color:#2A3A4A;font-size:10px">— broad knowledge</span></span><span><b style="color:#A8E6A3">● GPT-4</b> <span style="color:#2A3A4A;font-size:10px">— logical structure</span></span><span><b style="color:#3DFFC0">● Perplexity</b> <span style="color:#2A3A4A;font-size:10px">— live research</span></span><span><b style="color:#C084FC">● DeepSeek</b> <span style="color:#2A3A4A;font-size:10px">— STEM depth</span></span></div><div style="border-top:1px solid #111E2E;padding-top:12px;font-size:10px;color:#2A3A4A">← History in sidebar · Click any past discussion to resume it</div></div>',unsafe_allow_html=True)
        return

    if start and question.strip():
        disc_id=str(uuid.uuid4())
        st.session_state.update({"phase":1,"question":question.strip(),"r1":{},"r2":{},"synthesis":None,"followups":[],"context_summary":context_summary,"active_id":disc_id,"created_at":datetime.utcnow().isoformat()})
    ...
    st.markdown(f'<div style="background:rgba(61,255,192,0.05);border:1px solid rgba(61,255,192,0.25);border-radius:10px;padding:24px;font-family:monospace"><div style="display:flex;align-items:center;gap:10px;margin-bottom:14px"><div style="width:28px;height:28px;border-radius:50%;background:rgba(61,255,192,0.1);border:1.5px solid rgba(61,255,192,0.4);display:flex;align-items:center;justify-content:center;color:#3DFFC0">✦</div><div><div style="font-size:12px;font-weight:bold;color:#3DFFC0">SYNTHESIZED ANSWER</div><div style="font-size:10px;color:#2A5A4A">Claude · Gemini · GPT-4 · Perplexity · DeepSeek</div></div></div><div style="color:#C0D8D0;font-size:13.5px;line-height:1.8;white-space:pre-wrap">{safe}</div></div>',unsafe_allow_html=True)
    ...
    if st.session_state.phase>=4:
        ...
        for i,fu in enumerate(st.session_state.followups):
            with st.expander(f"↩ Follow-up {i+1}: {fu['question'][:55]}{'...' if len(fu['question'])>55 else ''}",expanded=(i==len(st.session_state.followups)-1)):
                cols=st.columns(5)
                ...
                if fu.get("synthesis"):
                    ...
                    st.markdown(f'<div style="background:rgba(123,167,255,0.05);border:1px solid rgba(123,167,255,0.2);border-radius:10px;padding:20px;margin-top:12px;font-family:monospace"><div style="font-size:11px;font-weight:bold;color:#7BA7FF;margin-bottom:10px">↩ UPDATED SYNTHESIS</div><div style="color:#C0D0E8;font-size:13px;line-height:1.8;white-space:pre-wrap">{sfu}</div></div>',unsafe_allow_html=True)
        ci,cs=st.columns([5,1])
        with ci: fu_q=st.text_input("fu",placeholder="Ask a follow-up, counter the synthesis, challenge a point...",label_visibility="collapsed",key=f"fu_{len(st.session_state.followups)}")
        with cs: send_fu=st.button("▶ SEND",use_container_width=True,key=f"send_{len(st.session_state.followups)}")
        if send_fu and fu_q.strip():
            ...
            with st.spinner("All 5 models responding..."):
                fu_results=run_parallel([(p,"followup",hctx,None) for p in PERSONAS_ORDER])
            fu_map=dict(zip(PERSONAS_ORDER,fu_results))
            with st.spinner("Synthesizing..."):
                try: fu_synth=call_claude(FOLLOWUP_SYNTH_SYSTEM,hctx+"\n\nFOLLOW-UP RESPONSES:\n"+"\n\n".join(f'[{AI_CONFIG[p]["name"]}]\n{fu_map[p]}' for p in PERSONAS_ORDER)+"\n\nSynthesize the best updated answer.")

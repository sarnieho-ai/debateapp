...
# ─── Storage Layer (Supabase with JSON fallback) ──────────────────
...
def _sb_load_history():
    try:
        ...
        r.raise_for_status()
        rows = r.json()
        # Deserialise JSONB columns stored as strings (safety net)
        for row in rows:
...
def phase_header(num, title, state):
    ...
    st.markdown(f'<div style="display:flex;align-items:center;gap:12px;margin:20px 0 14px 0">'
                f'<span style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:50%;background:{bg};border:1.5px solid {border};font-size:12px;font-weight:bold;color:{tc}">{icon}</span>'
                f'<span style="font-size:11px;font-weight:bold;letter-spacing:0.1em;color:{tc};font-family:monospace">{title}</span></div>',unsafe_allow_html=True)
...
def key_status_banner():
    ...
    db_icon = '<span style="color:#3DFFC0">✓ Supabase</span>' if STORAGE_MODE=="supabase" else '<span style="color:#C084FC">◎ Local JSON</span>'
    st.markdown(f'<div style="font-size:11px;padding:10px 14px;background:#090E18;border-radius:6px;border-left:3px solid #1A3A5A;margin-bottom:18px;font-family:monospace">'
                f'API &nbsp;·&nbsp; {" &nbsp;·&nbsp; ".join(parts)} &nbsp;·&nbsp; STORAGE &nbsp;·&nbsp; {db_icon}</div>',unsafe_allow_html=True)
...
def render_context_panel():
    ...
    if ctx_summary:
        st.markdown(f'<div style="font-size:10px;color:#3DFFC0;padding:6px 12px;background:rgba(61,255,192,0.05);border:1px solid rgba(61,255,192,0.2);border-radius:6px;margin-top:6px;font-family:monospace">✓ Context: {ctx_summary}</div>',unsafe_allow_html=True)
...
def copy_button_html(text_to_copy):
    ...
    style="width:100%;background:rgba(123,167,255,0.15);color:#7BA7FF;border:1px solid rgba(123,167,255,0.4);
           border-radius:6px;padding:9px 4px;font-family:monospace;font-size:11px;font-weight:bold;
           cursor:pointer;transition:all 0.2s;letter-spacing:0.08em">📋 COPY</button>
    """
...
def render_history_sidebar():
    with st.sidebar:
        ...
        st.markdown(f'<hr style="border:none;border-top:1px solid #0D1820;margin:20px 0 10px 0">'
                    f'<div style="font-size:9px;color:{mode_color};line-height:1.6;font-family:monospace">💾 {mode_label}</div>',unsafe_allow_html=True)
...
def main():
    ...
    if st.session_state.phase==0 and not start:
        st.markdown('<div style="padding:20px;background:#090E18;border-radius:10px;border:1px solid #111E2E;font-family:monospace">'
                    ...
                    '← History in sidebar · Click any past discussion to resume it</div></div>',unsafe_allow_html=True)
        return
    ...
    if not st.session_state.synthesis:
        ...
        with st.spinner("Synthesizing final answer..."):
            try: st.session_state.synthesis=call_claude(SYNTH_SYSTEM,sp)
    ...
    st.markdown(f'<div style="background:rgba(61,255,192,0.05);border:1px solid rgba(61,255,192,0.25);border-radius:10px;padding:24px;font-family:monospace">'
                ...
                f'<div style="width:28px;height:28px;border-radius:50%;background:rgba(61,255,192,0.1);border:1.5px solid rgba(61,255,192,0.4);display:flex;align-items:center;justify-content:center;color:#3DFFC0">✦</div>'
    ...
    if st.session_state.phase>=4:
        ...
        for i,fu in enumerate(st.session_state.followups):
            with st.expander(f"↩ Follow-up {i+1}: {fu['question'][:55]}{'...' if len(fu['question'])>55 else ''}",expanded=(i==len(st.session_state.followups)-1)):
                ...
                if fu.get("synthesis"):
                    ...
                    st.markdown(f'<div style="background:rgba(123,167,255,0.05);border:1px solid rgba(123,167,255,0.2);border-radius:10px;padding:20px;margin-top:12px;font-family:monospace">'
                                f'<div style="font-size:11px;font-weight:bold;color:#7BA7FF;margin-bottom:10px">↩ UPDATED SYNTHESIS</div>'
                                f'<div style="color:#C0D0E8;font-size:13px;line-height:1.8;white-space:pre-wrap">{sfu}</div></div>',unsafe_allow_html=True)
        n=len(st.session_state.followups)
        fu_q=st.text_input("fu",placeholder="Ask a follow-up, counter the synthesis, challenge a point...",label_visibility="collapsed",key=f"fu_{n}")
        with st.expander("📎  Add attachments to follow-up  (optional)",expanded=False):
        ...
        if send_fu and fu_q.strip():
            ...
            with st.spinner("Synthesizing updated answer..."):
                try: fu_synth=call_claude(FOLLOWUP_SYNTH_SYSTEM,fu_sp)
                except Exception as e: fu_synth=f"Synthesis error: {e}"
            st.session_state.followups.append({"question":fu_q.strip(),"responses":fu_map,"synthesis":fu_synth,"context_summary":fu_ctx_summary})
            save_discussion(_current_discussion()); st.rerun()
